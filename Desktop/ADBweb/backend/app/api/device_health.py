...
async def get_health_overview(
    ...
    """获取健康度总览"""
    # 获取所有设备的最新健康度（不限制在线状态）
    devices = db.exec(select(Device)).all()
    
    health_data = []
    for device in devices:
        statement = select(DeviceHealthRecord).where(
            DeviceHealthRecord.device_id == device.id
        ).order_by(DeviceHealthRecord.created_at.desc()).limit(1)
        ...
        if health_record:
            ...
            health_data.append({
                ...
                "device_name": device.model,
                "health_score": health_record.health_score,
                "level_code": level_code,  # 添加 level_code
                "level_name": level_name,
                "level_color": level_color,
                "battery_level": health_record.battery_level,
                "temperature": health_record.temperature,
                "cpu_usage": health_record.cpu_usage,  # 添加 CPU
                "memory_usage": health_record.memory_usage,  # 添加内存
                "storage_usage": health_record.storage_usage,  # 添加存储
                "network_status": health_record.network_status,  # 添加网络
                "last_check_time": health_record.created_at.isoformat()  # 添加检查时间
            })
