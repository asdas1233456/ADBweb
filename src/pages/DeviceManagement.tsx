...
const DeviceManagement = () => {
  ...
  const mapApiDevice = (device: any): Device => ({
    id: String(device.id ?? device.serial_number ?? device.serialNumber ?? ''),
    ...
    group_name: device.group_name ?? device.groupName,
    cpu_usage: device.cpu_usage,
    memory_usage: device.memory_usage,
    last_connected_at: device.last_connected_at ?? device.lastConnectedAt,
  })
  ...
  const loadDevices = async () => {
    ...
    try {
      ...
      const items = (response.items || []).map(mapApiDevice)
      setDevices(items)
      ...
      try {
        ...
        const groups = Array.from(new Set(items.map((d) => d.group_name).filter(Boolean))) as string[]
    ...
    if (value.trim()) {
      ...
      filtered = filtered.filter(
        (device) =>
          ...
          device.androidVersion?.toLowerCase().includes(searchLower)
      )
    }
  ...
  const handleConfirmBatchExecute = async () => {
    ...
    try {
      ...
      message.success(`已向 ${selectedRowKeys.length} 台设备发送执行任务`)
  ...
  const handleSaveGroup = async () => {
    ...
    try {
      for (const deviceId of selectedRowKeys) {
        await fetchWithAuth(
          `${API_BASE}/devices/${deviceId}/group?group_name=${encodeURIComponent(newGroupName)}`,
          { method: 'PUT' }
        )
      }
  ...
  const loadAvailableDevices = async () => {
    ...
    try {
      ...
      message.success('已扫描 ADB 设备')
      await loadDevices()

      const allDevicesResponse = await deviceApi.getList({ page: 1, page_size: 1000 })
      const items = (allDevicesResponse.items || []).map(mapApiDevice)
  ...
  const handleConfirmAddDevice = async (device: Device) => {
    try {
  ...
  const handleDisconnect = async (device: Device) => {
    try {
      await deviceApi.disconnect(Number(device.id))
  ...
  const handleDelete = async (device: Device) => {
    try {
      await fetchWithAuth(`${API_BASE}/devices/${device.id}`, { method: 'DELETE' })
  ...
  return (
    <div>
      <div style={{ marginBottom: 16, display: 'flex', justifyContent: 'space-between' }}>
        ...
        <Space data-tour="devices-actions">
          <Input
      ...
      {selectedRowKeys.length > 0 && (
        <Card
          data-tour="devices-batch"
          style={{ marginBottom: 16, background: '#e6f7ff', borderColor: '#91d5ff' }}
        >
          <Space>
            <span>已选择 {selectedRowKeys.length} 台设备</span>
            <Button type="primary" size="small" icon={<PlayCircleOutlined />} onClick={handleBatchExecute}>
              批量执行
            </Button>
            <Button size="small" icon={<AppstoreOutlined />} onClick={handleBatchSetGroup}>
              设置分组
            </Button>
      ...
      >
        <Spin spinning={loading}>
          <Table
            ...
            locale={{
              emptyText: '暂无设备，请先连接 ADB 设备或点击“刷新”',
            }}
          />
        </Spin>
      </Card>
      ...
      >
        ...
        >
          <div style={{ textAlign: 'center' }}>
            ...
            <div>投屏功能开发中...</div>
            <div style={{ fontSize: 14, marginTop: 8 }}>设备: {selectedDevice?.model}</div>
          </div>
        </div>
      </Modal>
          ...
          <div style={{ padding: '16px 0' }}>
            <Card size="small" style={{ marginBottom: 16 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '120px 1fr', gap: '12px 16px' }}>
                ...
                <div style={{ color: '#666' }}>分辨率</div>
                <div>{selectedDevice.resolution || '-'}</div>

                <div style={{ color: '#666' }}>电量</div>
                <div>
                  <Progress
      ...
      >
        <div style={{ marginBottom: 16 }}>
          <div style={{ marginBottom: 8, color: '#666' }}>已选择 {selectedRowKeys.length} 台设备</div>
          <Select
        ...
        <div style={{ marginBottom: 16 }}>
          <div style={{ padding: '12px', background: '#e6f7ff', borderRadius: 4, marginBottom: 16 }}>
            ...
            <span style={{ color: '#666' }}>
              点击“重新扫描”会自动检测并添加通过 ADB 连接的设备，请确保设备已连接并开启 USB 调试。
            </span>
          </div>
        </div>
            ...
            <div style={{ textAlign: 'center', padding: '60px 20px', color: '#999' }}>
              ...
              <div style={{ fontSize: 14 }}>
                ...
                <div style={{ marginTop: 12, textAlign: 'left', display: 'inline-block' }}>
                  <div>1. 设备是否通过 USB 连接到电脑</div>
                  <div>2. 设备是否开启 USB 调试模式</div>
                  <div>3. 是否已安装 ADB 驱动</div>
                  <div>4. 点击“重新扫描”刷新设备列表</div>
                </div>
              </div>
            </div>
          ) : (
            <div>
              <div style={{ marginBottom: 12, color: '#666' }}>当前系统中有 {availableDevices.length} 台设备：</div>
              <div style={{ maxHeight: 400, overflowY: 'auto' }}>
                {availableDevices.map((device) => (
                  <Card key={device.id} size="small" style={{ marginBottom: 12 }} hoverable>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: 16, fontWeight: 500, marginBottom: 8 }}>{device.model}</div>
                        <div
                          style={{
                            display: 'grid',
                        ...
                        <div style={{ marginTop: 8 }}>{getStatusTag(device.status)}</div>
                      </div>
              ...
              <div style={{ marginTop: 16, padding: '12px', background: '#f0f0f0', borderRadius: 4, fontSize: 14, color: '#666' }}>
                ...
                提示：所有通过 ADB 连接的设备会自动添加到系统中。如需添加新设备，请连接设备后点击“重新扫描”。
              </div>
            </div>
          )}
        </Spin>
